...
void ENV_Task_Entry(void *argument)
{
  ...
  /* 针对PCB时序裕量的默认调参 */
  dhtCfg.sample_us = 35U;         /* 兼容保留参数 */
  dhtCfg.max_retries = 3U;
  dhtCfg.bit_timeout_us = 160U;
  dhtCfg.resp_timeout_us = 300U;
  dhtCfg.high_threshold_us = 18U; /* 脉宽判1阈值（计数单位近似us） */

  DHT11_Init();
  DHT11_SetConfig(&dhtCfg);
  DHT11_ResetStats();
  BEEP_Init(); /* ???????????????? */
  osDelay(1200);
